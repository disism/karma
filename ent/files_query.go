...
func (fq *FilesQuery) QuerySaves() *SavesQuery {
	query := (&SavesClient{config: fq.config}).Query()
...
func (fq *FilesQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Files, error) {
	...
	if query := fq.withSaves; query != nil {
		if err := fq.loadSaves(ctx, query, nodes,
			func(n *Files) { n.Edges.Saves = []*Saves{} },
			func(n *Files, e *Saves) { n.Edges.Saves = append(n.Edges.Saves, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FilesQuery) loadSaves(ctx context.Context, query *SavesQuery, nodes []*Files, init func(*Files), assign func(*Files, *Saves)) error {
	...
	query.Where(predicate.Saves(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(files.SavesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.files_saves
		if fk == nil {
			return fmt.Errorf(`foreign-key "files_saves" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "files_saves" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
