...
func (uq *UsersQuery) QueryDirs() *DirsQuery {
	query := (&DirsClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UsersQuery) QuerySaves() *SavesQuery {
	query := (&SavesClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UsersQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Users, error) {
	...
	if query := uq.withDirs; query != nil {
		if err := uq.loadDirs(ctx, query, nodes,
			func(n *Users) { n.Edges.Dirs = []*Dirs{} },
			func(n *Users, e *Dirs) { n.Edges.Dirs = append(n.Edges.Dirs, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withSaves; query != nil {
		if err := uq.loadSaves(ctx, query, nodes,
			func(n *Users) { n.Edges.Saves = []*Saves{} },
			func(n *Users, e *Saves) { n.Edges.Saves = append(n.Edges.Saves, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UsersQuery) loadDirs(ctx context.Context, query *DirsQuery, nodes []*Users, init func(*Users), assign func(*Users, *Dirs)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Users)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Dirs(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(users.DirsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.users_dirs
		if fk == nil {
			return fmt.Errorf(`foreign-key "users_dirs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "users_dirs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UsersQuery) loadSaves(ctx context.Context, query *SavesQuery, nodes []*Users, init func(*Users), assign func(*Users, *Saves)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Users)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Saves(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(users.SavesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.users_saves
		if fk == nil {
			return fmt.Errorf(`foreign-key "users_saves" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "users_saves" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
